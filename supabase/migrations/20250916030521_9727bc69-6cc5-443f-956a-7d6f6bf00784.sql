-- Create comprehensive employee reporting function that combines tasks, time tracking, projects, and metrics
CREATE OR REPLACE FUNCTION public.get_comprehensive_employee_report(
  target_user_id uuid,
  start_date date,
  end_date date,
  report_granularity text DEFAULT 'daily'
)
RETURNS TABLE (
  period_date date,
  period_label text,
...
-- Create enhanced daily task completion function
CREATE OR REPLACE FUNCTION public.get_employee_daily_task_completion(
  target_user_id uuid,
  start_date date,
  end_date date
)
