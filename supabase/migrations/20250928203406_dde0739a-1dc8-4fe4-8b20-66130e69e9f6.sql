-- Fix create_inventory_transaction_from_receipt_line function to use correct inventory_transactions schema
...
BEGIN
  ...
  SELECT 
    ...
    wr.vendor_name,     -- Use vendor_name
    wr.received_at,     -- Use received_at
    wr.posted_by        -- Get the user who posted
  ...
  -- Create inventory transaction using CORRECT column names from actual schema
  INSERT INTO inventory_transactions (
    ...
    total_cost,
    transaction_date,
    reference_number,        -- Use reference_number instead of reference_type
    receipt_line_id,         -- Use receipt_line_id to link to receipt line
    notes,
    user_id,                 -- Use user_id instead of created_by
    created_by,              -- Also set created_by
    vendor_name,
    po_number
  ) VALUES (
    ...
    'WR-' || receipt_line_record.receipt_id::text,  -- Use reference_number
    p_receipt_line_id,       -- Link to receipt line
    'Inventory received from vendor: ' || COALESCE(receipt_line_record.vendor_name, 'Unknown'),
    COALESCE(receipt_line_record.posted_by, auth.uid()),  -- Use user_id
    COALESCE(receipt_line_record.posted_by, auth.uid()),  -- Also set created_by
    receipt_line_record.vendor_name,
    null  -- po_number can be null for now
  );
  
  -- Update or insert warehouse item stock levels (separate from inventory_transactions)
