-- Fix the function to use NULL for posted_by instead of invalid UUID
...
BEGIN
  ...
  -- Update receipt status to posted (allow NULL for posted_by if no user)
  UPDATE warehouse_receipts
  SET status = 'posted', 
      posted_at = now(),
      posted_by = current_user_id, -- Can be NULL if no authenticated user
  ...
  LOOP
    ...
    -- Create inventory transaction record (handle null user_id with system fallback)
    INSERT INTO inventory_transactions (
      ...
      notes,
      transaction_date,
      user_id, -- Must be valid UUID, use system user if needed
      created_by,
      team_id,
      organization_id,
      created_at
    ...
    VALUES (
      ...
      COALESCE(current_user_id, '00000000-0000-0000-0000-000000000000'::uuid), -- System user fallback
      current_user_id,
      receipt_record.warehouse_id,
      current_org_id,
      now()
