-- Create receive_stock function for warehouse receiving with lot tracking
...
BEGIN
  ...
  INSERT INTO public.warehouse_items (
    warehouse_id, item_id, on_hand, available, allocated, organization_id
  )
  VALUES (
    p_warehouse_id, p_item_id, p_quantity, p_quantity, 0, v_org_id
  )
  ON CONFLICT (warehouse_id, item_id) 
  DO UPDATE SET
    on_hand = warehouse_items.on_hand + p_quantity,
    available = warehouse_items.available + p_quantity,
    updated_at = now();
