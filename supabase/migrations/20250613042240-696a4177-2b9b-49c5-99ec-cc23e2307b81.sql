...
-- Remove any remaining conflicting legacy RLS policies on the projects table
...
-- Also drop the current policies to recreate them cleanly
...
-- Create fresh, clean policies for the projects table using the working get_current_user_organization_id() function
-- Policy 1: SELECT - Users can view projects from their organization
CREATE POLICY "projects_org_select_fixed" 
ON public.projects 
FOR SELECT 
TO authenticated
...
CREATE POLICY "projects_org_insert_fixed" 
ON public.projects 
FOR INSERT 
TO authenticated
WITH CHECK (organization_id = public.get_current_user_organization_id());

-- Policy 3: UPDATE - Project managers, team members, and admins can update projects
CREATE POLICY "projects_org_update_fixed" 
ON public.projects 
FOR UPDATE 
TO authenticated
USING (
...
-- Policy 4: DELETE - Project managers and admins can delete projects
CREATE POLICY "projects_org_delete_fixed" 
ON public.projects 
FOR DELETE 
TO authenticated
USING (
