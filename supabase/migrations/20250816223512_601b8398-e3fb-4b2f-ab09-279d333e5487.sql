...
-- If it already exists, this will fail; so we create it conditionally.
-- Supabase migrations run in a single transaction; use DO block for conditional create.
...
begin
  if not exists (
    ...
    execute $$
      create policy users_self_update_only_restrictive
      as restrictive
      on public.users
      for update
      to authenticated
