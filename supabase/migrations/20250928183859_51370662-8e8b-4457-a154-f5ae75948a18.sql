-- Fix column ambiguity issues by using proper table aliasing
...
DECLARE
  current_org_id uuid;
BEGIN
  -- Get organization from current user context using proper table aliasing
  SELECT users.organization_id INTO current_org_id FROM users WHERE users.id = auth.uid();
