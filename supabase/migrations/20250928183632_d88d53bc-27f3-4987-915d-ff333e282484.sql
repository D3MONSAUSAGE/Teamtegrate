-- Clean up duplicate database functions and fix parameter mismatches

-- Drop all existing duplicate functions
...
-- Create single, clean get_daily_movements function that matches service calls
...
BEGIN
  ...
  SELECT organization_id INTO current_org_id FROM users WHERE id = auth.uid();
...
-- Create warehouse-specific daily movements function
...
-- Create real-time inventory value function that matches service calls
