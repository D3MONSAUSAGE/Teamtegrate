-- Fix handle_new_user to safely handle NULL/empty invited_team_id
...
DECLARE
  ...
  invited_team_id_text TEXT; -- Temporary variable for safe casting
  team_role TEXT;
BEGIN
  ...
  IF invite_code IS NOT NULL THEN
    ...
    IF invite_result->>'success' = 'true' THEN
      ...
      -- FIXED: Safely handle team_id conversion
      invited_team_id_text := invite_result->>'invited_team_id';
      IF invited_team_id_text IS NOT NULL AND invited_team_id_text != '' THEN
        invited_team_id := invited_team_id_text::UUID;
      ELSE
