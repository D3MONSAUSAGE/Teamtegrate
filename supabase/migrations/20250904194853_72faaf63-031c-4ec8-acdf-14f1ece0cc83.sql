-- Function to handle meeting response notifications
...
-- Create trigger for meeting response notifications
DROP TRIGGER IF EXISTS meeting_response_notification_trigger ON public.meeting_participants;

CREATE TRIGGER meeting_response_notification_trigger
  AFTER UPDATE ON public.meeting_participants
  FOR EACH ROW
  EXECUTE FUNCTION public.notify_organizer_of_meeting_response();

-- Add new notification types if they don't exist
DO $$
BEGIN
  -- We don't need to create an enum, the type column is just text
  -- The new notification types will be: meeting_response_accepted, meeting_response_declined, meeting_response_tentative
END $$;