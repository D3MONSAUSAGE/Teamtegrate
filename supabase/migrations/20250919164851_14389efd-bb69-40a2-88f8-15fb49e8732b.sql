...
-- Create a corrected function that uses the actual field names (assigned_to_names not assigned_to_name)
...
BEGIN
  ...
  IF (TG_OP = 'UPDATE' AND (
    ...
    -- Only insert audit log if compliance_audit_logs table exists
...
-- Recreate the trigger with the corrected function (only if needed for auditing)
-- Comment out for now since it might not be essential
