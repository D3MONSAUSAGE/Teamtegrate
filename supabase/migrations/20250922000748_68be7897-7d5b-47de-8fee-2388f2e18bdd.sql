-- Enhanced Centralized Checklist System Migration (Fixed)
...
-- Functions for display codes
CREATE OR REPLACE FUNCTION get_template_code(template_id UUID) 
RETURNS TEXT AS $$
DECLARE
  created_at TIMESTAMPTZ;
BEGIN
  SELECT t.created_at INTO created_at FROM checklist_templates_v2 t WHERE t.id = template_id;
  RETURN CONCAT('CLT-', TO_CHAR(created_at,'YYYY'), '-', LPAD((EXTRACT(EPOCH FROM created_at)::BIGINT % 10000)::TEXT, 4, '0'));
END;
$$ LANGUAGE plpgsql STABLE;

CREATE OR REPLACE FUNCTION get_instance_display_code(instance_id UUID) 
RETURNS TEXT AS $$
DECLARE
  date_val DATE;
BEGIN
  SELECT i.date INTO date_val FROM checklist_instances_v2 i WHERE i.id = instance_id;
  RETURN 'CHK-' || TO_CHAR(date_val,'YYYYMMDD') || '-' || SUBSTR(REPLACE(instance_id::TEXT,'-',''),1,6);
END;
$$ LANGUAGE plpgsql STABLE;
