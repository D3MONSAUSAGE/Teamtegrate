-- Create onboarding document requirements and submissions system
...
CREATE TABLE IF NOT EXISTS public.onboarding_document_submissions (
  ...
  is_overdue BOOLEAN GENERATED ALWAYS AS (due_date IS NOT NULL AND due_date < now() AND submission_status NOT IN ('approved')) STORED,
  created_at TIMESTAMP WITH TIME ZONE DEFAULT now(),
  updated_at TIMESTAMP WITH TIME ZONE DEFAULT now()
);
...
CREATE TABLE IF NOT EXISTS public.onboarding_compliance_items (
  ...
  completed_by UUID,
  notes TEXT,
  is_overdue BOOLEAN GENERATED ALWAYS AS (due_date IS NOT NULL AND due_date < now() AND status NOT IN ('completed', 'not_applicable')) STORED,
  created_at TIMESTAMP WITH TIME ZONE DEFAULT now(),
  updated_at TIMESTAMP WITH TIME ZONE DEFAULT now()
);
