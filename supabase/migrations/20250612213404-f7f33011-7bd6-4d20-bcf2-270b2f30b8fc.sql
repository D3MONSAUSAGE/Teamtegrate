...
-- Complete Projects Table RLS Policy Cleanup - Remove ALL remaining conflicting policies

-- Drop all remaining old policies that are still causing conflicts
...
-- Also drop the new policies we created in the previous migration so we can recreate them cleanly
...
-- Now create ONLY our 4 clean policies for the projects table
-- Policy 1: SELECT - Users can view projects from their organization
CREATE POLICY "projects_org_select" 
ON public.projects 
FOR SELECT 
TO authenticated
...
CREATE POLICY "projects_org_insert" 
ON public.projects 
FOR INSERT 
TO authenticated
