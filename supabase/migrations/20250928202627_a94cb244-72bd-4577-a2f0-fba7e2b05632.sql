-- Fix the create_inventory_transaction_from_receipt_line function to get organization_id from warehouses table
...
BEGIN
  -- Get receipt line details with joins - FIX: get organization_id from warehouses table
  SELECT 
    wrl.*,
    wr.warehouse_id,
    w.organization_id,  -- Get from warehouses table, not warehouse_receipts
    wr.supplier_id,
    wr.received_date
  INTO receipt_line_record
  FROM warehouse_receipt_lines wrl
  JOIN warehouse_receipts wr ON wrl.receipt_id = wr.id
  JOIN warehouses w ON wr.warehouse_id = w.id  -- Add join to warehouses table
