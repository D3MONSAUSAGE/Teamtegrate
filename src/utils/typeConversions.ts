...
export const flatProjectsToProjects = (flatProjects: FlatProject[]): Project[] => {
  return flatProjects.map(flatProject => ({
    ...
    startDate: new Date(flatProject.createdAt),
    endDate: new Date(flatProject.updatedAt),
    managerId: flatProject.managerId,
    createdAt: new Date(flatProject.createdAt),
    updatedAt: new Date(flatProject.updatedAt),
    teamMemberIds: [],
    budget: 0,
    budgetSpent: 0,
    isCompleted: false,
