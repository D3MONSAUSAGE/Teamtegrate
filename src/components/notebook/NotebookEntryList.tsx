...
import { NotebookEntry } from "@/pages/NotebookPage";
...
const NotebookEntryList: React.FC<NotebookEntryListProps> = ({
  entries,
  isLoading,
  onChange
}) => {
  ...
  return (
    <div className="flex flex-col gap-4">
      {isLoading ? (
        ...
        <div className="text-muted-foreground px-2">No notebook entries.</div>
      ) : (
        entries.map(entry => (
          <div
            key={entry.id}
...
export default NotebookEntryList;
