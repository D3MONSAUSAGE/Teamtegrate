...
const MetricsCommandCenter: React.FC<MetricsCommandCenterProps> = ({ tasks, dailyScore }) => {
  ...
  // Show loading state if no tasks data
  if (!tasks || tasks.length === 0) {
    return (
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-4 mb-8">
        {Array.from({ length: 6 }).map((_, index) => (
          <Card key={index} className="border-0 bg-white shadow-sm ring-1 ring-slate-200/50">
...
export default React.memo(MetricsCommandCenter);
